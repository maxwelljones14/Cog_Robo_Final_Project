...
def onclick(event, prediction):
    ...
    if prediction is not None:
        print(f"distance to the location you clicked is {round(prediction[int(round(iy, 0)), int(round(ix, 0))] / 10, 3)} cm")
    else:
...
#    print("image being updated")
#    old_image = ima, = model.forward(sample)
#        prediction = (
...
#    return depth
  ...
  class DisplayCamera(StateNode):
    def start(self,event=None):
        super().start(event)
        old_image = self.robot.world.latest_image.raw_image #get the image from the camera world
...
#             print(image_box.top_left_y, image_box.height, middlex, lowy)
            image_box_vals.append((middlex, lowy))

        # print(f"top left x: {image_box.top_left_x}, top left right: {image_box.top_left_y}, height: {image_box.height}, width: {image_box.width}")
        ...
        for cubeseen in cubes:
            ...
            distance = (translated_point[0][0] **2 + translated_point[1][0] **2 + translated_point[2][0] ** 2) ** .5
        ...
        cube_depth_estimates = [] #compute current depth estimates at all locations of cubes (note these values are roughly between 0 and 1)
        for middlex, lowy in image_box_vals:
            cube_depth_estimates.append(prediction_inverted[int(round(prediction.shape[0] - lowy, 0)), int(round(middlex, 0))])
        
        # compute mathematically optimal scaling factor
        dot_product = sum([distances_to_cubes[x] * cube_depth_estimates[x] for x in range(len(cube_depth_estimates))])
        magnitude = sum([cube_depth_estimates[x] ** 2 for x in range(len(cube_depth_estimates))])
        if magnitude != 0:
        ...
        prediction_normalized = prediction_inverted * scaling_factor #compute actual depth guesses
        ...
        for middlex, lowy in image_box_vals:
            for x in range(7):
                for y in range(7):
                    prediction[prediction.shape[0] - int(round(lowy, 0))  + 3 - y, int(round(middlex, 0)) + 3 - x] = 0;
#                     print(f"middle index: {middlex}, lowy: {lowy}")

        depth = write_depth(prediction)
        cv2.imwrite("im.png", depth)
        fig = plt.figure(1)
        # add functionality so that a click from a user results in depth prediction 
        cid = fig.canvas.mpl_connect('button_press_event', lambda x: onclick(x, prediction_normalized))
        plt.imshow(depth)

        # if(middlex is not None and lowy is not None):
        #     plt.scatter(np.array([middlex]), np.array([prediction.shape[0] - lowy]))
  ...
  def setup(self):
      #     start: StateNode() =TM=> display
      #     display: self.DisplayCamera() =T(.1)=> display
      # 
